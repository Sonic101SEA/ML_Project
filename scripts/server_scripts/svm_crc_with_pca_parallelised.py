...
X_train, X_test, y_train, y_test = train_test_split(reduced, img_labels_integer, stratify = img_labels_integer, test_size=0.2, random_state=8)
...
model = GridSearchCV(svc, param_grid = param_grid, n_jobs = 4)
...
with open("svm_pca_parallelised.txt", "w") as external_file:
...
plt.show()
plt.savefig('svm_confusion_crc.pdf')
